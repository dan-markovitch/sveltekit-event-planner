...
<div class="max-w-xl">
    {#if data.event}
        {#if !editing}
            {#if saved}
                <p class="text-green-600 text-sm bg-green-50 border border-green-200 rounded-lg px-4 py-3 mb-4">
            ...
            <div class="flex items-center gap-3">
                ...
                <form method="POST" action="?/delete" use:enhance={() => {
                    ...
                    <button
                        ...
                        disabled={submitting}
                        onclick={(e) => {
                            if (!confirm(`Are you sure you want to delete "${data.event?.title}"? This cannot be undone.`)) {
                                e.preventDefault();
                            }
                        }}
                ...
                <a href="/" class="text-sm text-gray-500 hover:text-gray-700 hover:underline ml-2">← Back to Events</a>
            </div>
